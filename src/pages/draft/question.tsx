...
const Question: React.FC<{}> = () => {
  ...
  return <>
    <Card title="1" style={{width: '49%', marginLeft: '1%'}} actions={[ <PlusOutlined onClick={() => edit('add')} key="setting" /> ]}>
    {
      questionList.map((item, index) => {
        return <Card key={index}>
                <Row align="middle">
                  <Col span={18}><h2>{index + 1}. {item.title}</h2></Col>
                  <Col span={6}>
                    <Button type="link" onClick={() => edit('edit', index)} size="small">修改</Button>
                  </Col>
                </Row>
                {
                  Array.isArray(item.options) 
                   ? item.options.map((each, index) => {
                      ...
                      return <Row key={index}>
                        <Col style={{backgroundColor: `${code === item.rightAnswer ? '#1890ff' : '#FFF'}`}} span={12}>{code}、{each}</Col>
                      </Row>
                    })
                  : ''
                }
